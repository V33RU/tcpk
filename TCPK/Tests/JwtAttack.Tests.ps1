...
BeforeAll {
    ...
    $script:job = Start-Job -ArgumentList $script:port, $script:pubPem, $script:qx, $script:qy, $script:kidFile -ScriptBlock {
        param($port, $pubPem, $qx, $qy, $kidFile)
            ...
            while ($true) {
                ...
                if ($tok) {
                    ...
                    switch -Wildcard ($path) {
                        ...
                        '/jku'       { if ($j -and "$($j.h.alg)" -eq 'RS256') { $acc = $false; if ($j.h.jku) { $jp = Join-Path ([System.IO.Path]::GetTempPath()) 'tcpk-jku-jwks.json'; if (Test-Path -LiteralPath $jp) { try { $jwks = Get-Content -LiteralPath $jp -Raw | ConvertFrom-Json; $k0 = $jwks.keys[0]; $rp2 = New-Object Security.Cryptography.RSAParameters; $rp2.Modulus = b64d $k0.n; $rp2.Exponent = b64d $k0.e; $rj = [Security.Cryptography.RSA]::Create(); $rj.ImportParameters($rp2); if ($rj.VerifyData([Text.Encoding]::ASCII.GetBytes($j.si), (b64d $j.parts[2]), [Security.Cryptography.HashAlgorithmName]::SHA256, [Security.Cryptography.RSASignaturePadding]::Pkcs1)) { $acc = $true }; $rj.Dispose() } catch {} } }; if (-not $acc) { try { $srv = [Security.Cryptography.RSA]::Create(); $srv.ImportFromPem($pubPem); if ($srv.VerifyData([Text.Encoding]::ASCII.GetBytes($j.si), (b64d $j.parts[2]), [Security.Cryptography.HashAlgorithmName]::SHA256, [Security.Cryptography.RSASignaturePadding]::Pkcs1)) { $acc = $true }; $srv.Dispose() } catch {} }; if ($acc) { $ok = $true; $body = 'JKU-PROTECTED' } } }
                    }
                }
                if ($ok) { $status = 200 }
