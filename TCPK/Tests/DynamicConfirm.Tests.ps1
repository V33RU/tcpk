...
Describe 'Invoke-TcpkDynamicConfirm gating' {
    ...
    It 'throws without -ConfirmDynamic even when enabled' {
        { Invoke-TcpkDynamicConfirm -Target $script:safeExe } | Should -Throw
    }
}

Describe 'Invoke-TcpkDynamicConfirm observation' {
    It 'Confirmed (dynamic) when the app follows the CLI host override' {
    ...
    It 'stays Inferred/inconclusive when the app ignores the override' {
