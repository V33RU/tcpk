...
.DESCRIPTION
    Walks the vendor/product registry subtree (HKCU + HKLM) and inspects every
    value's DATA for secret material: passwords, API keys, tokens, connection
    strings, private-key blocks, and long base64 blobs under secret-suggestive
    value names. Registry-stored secrets are readable by the user (HKCU) or by
    anyone (HKLM) and survive uninstall.

.PARAMETER NameLike
    Vendor/product substring to locate the subtree (e.g. Acme, YourApp).
...
#>
    ...
    foreach ($r in $roots) {
        ...
        $vendorRoots = Get-ChildItem -Path $r -ErrorAction SilentlyContinue | Where-Object { $_.PSChildName -like "*$NameLike*" }
