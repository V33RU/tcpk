...
$script:TcpkTasvsMap = @(
    ...
    @{ rx='^(registry|autostart|scheduledtask|wmipersistence|ifeo|apppaths|shimcache|unquoted|service|servicebin|taskbin|acl\.|install-?dir|folderacls|programdata|kerneldrivers|driver|dllsearch|proxydll|sxs|firewall|avexclusion|uac)'; tasvs=@('TASVS-PLATFORM Platform Interaction'); da=@('DA6 Security Misconfiguration','DA5 Improper Authorization') }
    ...
    @{ rx='^electron\.v8-bytecode';                                                         tasvs=@('TASVS-CODE Code Quality and Build Settings'); da=@('DA6 Security Misconfiguration') }
    ...
    # servicedll.* is already reached by the '|service|' alternation on the platform line
    ...
    @{ rx='^strings\.';                                                                     tasvs=@('TASVS-CODE Code Quality & Build Settings'); da=@('DA6 Security Misconfiguration') }
