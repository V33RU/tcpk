...
- **Test-TcpkArchiveExpansion** - A75. Shipped archives whose expansion ratio makes them a resource-exhaustion hazard.
...
- **Test-TcpkBlePairing** - A56. BLE pairing model. Extends A50 to report `DevicePairingProtectionLevel.None` (HIGH), `Encryption` without `EncryptionAndAuthentication` (MEDIUM), `PairAsync` with no protection literal (MEDIUM), `ConfirmOnly`-only DevicePairingKinds (HIGH), auto-accept via zero-arg `Accept()` in `PairingRequested` handler (HIGH), hardcoded passkey / 128-bit key material near BLE call sites (HIGH), 32feet.NET `Confirm=true` + no address check (HIGH), Qt `pairingConfirmation(true)` (HIGH), native `BLUETOOTH_MITM_PROTECTION_NOT_REQUIRED` (MEDIUM), and Nordic/TI/Silabs SDK legacy-pairing markers (MEDIUM). Gated on a BLE anchor in the same file to keep unrelated DevicePairingProtectionLevel usage (Wi-Fi Direct, Miracast) out.
...
- **Test-TcpkDependencyConfusion** - C29. Dependency confusion attack surface in package manager configuration.
...
- **Test-TcpkDotenvSecrets** - A60. Parses shipped `.env` / `.env.local` / `.env.production` files as `KEY=VALUE` and flags secrets by KEY NAME rather than value entropy - closes the low-entropy blind spot in Test-TcpkEntropySecrets. Rules: `cfg.dotenv-secret-name` HIGH per line whose key matches a curated pattern (`*_PASSWORD`, `*_SECRET`, `*_API_KEY`, `AWS_SECRET_ACCESS_KEY`, `STRIPE_SECRET_KEY`, `JWT_SECRET`, ...) with a non-placeholder value, `cfg.dotenv-secret-url-embedded` CRITICAL when a value is a URL containing `user:pass@`, `cfg.dotenv-shipped-in-release` LOW inventory rule for every `.env` in the tree (downgrades for `.env.example` / `.env.sample` / `.env.template`). Values are masked in evidence.
...
- **Test-TcpkElectronFuses** - A42. Electron Fuses audit: parses the @electron/fuses wire from the app binary and flags insecure fuse states as Confirmed facts (EnableCookieEncryption off = plaintext cookies; RunAsNode / EnableNodeCliInspectArguments on = node-exec / --inspect LOLBins; EnableNodeOptionsEnvironmentVariable on = NODE_OPTIONS=--require ./x.js LPE under the signed app identity; EnableEmbeddedAsarIntegrityValidation off = tamperable app.asar) plus a full fuse-posture summary.
...
- **Test-TcpkFirmwareManifest** - A55. Parses JSON firmware manifests (files that name both a version and a download URL) and reports whether they carry a signature, an integrity hash, and downgrade protection. Rules: `plaintext-url` (HIGH), `no-signature` (HIGH), `hash-only` (MEDIUM), `weak-hash` (HIGH -- MD5/SHA1/CRC32/xxhash), `no-rollback-guard` (LOW). Static complement to K25 Invoke-TcpkFirmwarePlantProbe.
...
- **Test-TcpkIotCloudCreds** - A63. AWS IoT Core Thing certificates and Azure IoT Hub connection strings shipped in the artifact.
...
- **Test-TcpkJsSourceMap** - A61. JavaScript source-map exposure. Rules: `js.sourcemap.shipped` MEDIUM (a `.js.map` file ships alongside a shipped `.js`; DevTools reconstructs the original TypeScript / JSX including comments, private class names and inline endpoints), `js.sourcemap.comment-inline` MEDIUM (a shipped `.js` has a `//# sourceMappingURL=` comment), `js.sourcemap.datauri` HIGH (the sourceMappingURL is `data:application/json;base64,...` - pre-bundle source embedded verbatim in the shipped `.js`). Skips known third-party vendor paths (`node_modules/@types|@babel|react|electron|...`, `site-packages/`, `resources/electron/`, `vendor/`) so the report stays scoped to first-party JS.
- **Test-TcpkJwt** - A14. Embedded JSON Web Token (JWT) discovery + weakness analysis.
- **Test-TcpkMarkOfTheWeb** - FS02. Whether content the application brings in from outside carries the zone mark.
- **Test-TcpkMqttSurface** - A57. MQTT client surface across MQTTnet, uPLibrary.Networking.M2Mqtt, System.Net.Mqtt (archived Xamarin), paho.mqtt (Python), Eclipse Paho C, and esp-mqtt. Rules: `mqtt.cleartext-uri` (HIGH), `mqtt.tls-explicit-disable` (HIGH), `mqtt.mqttnet-tls-absent` (HIGH), `mqtt.paho-python-tls-absent` (HIGH), `mqtt.tls-verify-off` (HIGH), `mqtt.hardcoded-credentials` (CRITICAL, `WithCredentials("u","p")` literal pair), `mqtt.static-clientid` (HIGH), `mqtt.root-wildcard-subscribe` (HIGH), `mqtt.esp-transport-tcp` (HIGH), `mqtt.paho-c-nontls-build` (HIGH, `paho-mqtt3c.dll` without the TLS sibling), `mqtt.systemnetmqtt-unmaintained` (MEDIUM), `mqtt.no-keepalive` (LOW). Every rule gated on a per-file MQTT library anchor.
...
- **Test-TcpkNugetConfigCreds** - A62. Shipped `nuget.config` credential leak. Rules: `supply.nuget.cleartext-password` CRITICAL (a `<packageSourceCredentials>` block carries `<add key="ClearTextPassword" value="..."/>`), `supply.nuget.dpapi-password` HIGH (DPAPI-encrypted `<add key="Password" value="..."/>` - only reversible on the origin machine + user context, but shipped regardless), `supply.nuget.private-feed` LOW inventory rule for every `<packageSources>` entry that isn't `api.nuget.org` (helpful scope info; combine with the credential rules for the full private-feed supply-chain picture).
- **Test-TcpkOAuthState** - C35. OAuth 2.0 state parameter CSRF bypass and implicit flow detection.
...
- **Test-TcpkPasswordPolicy** - A74. Local password and lockout policy declared in shipped configuration.
- **Test-TcpkPdbPathLeak** - A59. Reads the PE debug directory's CodeView RSDS record and emits per-shape findings for the PdbPath stamped into the shipped .exe/.dll (present even when the .pdb is not shipped). Rules: `pe.debug.pdb-path-userprofile` MEDIUM (path contains C:\Users\<name>\), `pe.debug.pdb-path-unc` MEDIUM (UNC path leaks build-server hostname), `pe.debug.pdb-path-repo` LOW (path has \src\ / \repo\ / \projects\ segments), `pe.debug.pdb-path-leak` LOW (any PdbPath present). Distinct from Test-TcpkDevArtifacts which only fires when the .pdb file itself ships.
...
- **Test-TcpkPlatformAuth** - A73. Windows Hello / platform authentication: whether biometric is a cryptographic gate or a boolean.
- **Test-TcpkProvisioningPoP** - A58. IoT provisioning Proof-of-Possession material shipped inside a Windows companion app. Covers ESP-IDF Wi-Fi provisioning (`prov.esp.security0` HIGH, `prov.esp.pop-hardcoded` HIGH, `prov.esp.default-pop-sentinel` HIGH for the sample `abcd1234`, `prov.esp.security2-creds-hardcoded` HIGH), AWS IoT Fleet Provisioning (`prov.aws.claim-cert-shipped` HIGH when a `claim.crt` + `claim.key` pair AND a `$aws/certificates/create` topic anchor both appear, `prov.aws.fleet-topic` MEDIUM), Azure DPS (`prov.azure.dps-symkey-hardcoded` HIGH for `SecurityProviderSymmetricKey` + a 32/64-byte base64 literal), Matter commissioning (`prov.matter.passcode-hardcoded` HIGH, `prov.matter.sample-pin` HIGH for CHIP SDK samples 20202021/12345678, `prov.matter.qr-shipped` MEDIUM), Zigbee (`prov.zigbee.default-tclk` CRITICAL for `ZigBeeAlliance09`, `prov.zigbee.install-code-hardcoded` HIGH), and BLE SMP Just-Works fallback (`prov.ble.just-works-fallback` MEDIUM).
...
- **Test-TcpkReparseLoops** - NTFS junction / reparse-point loops under a target that can hang a naive recursive walker.
...
- **Test-TcpkSqlInjection** - C31. SQL injection via string concatenation in source code and embedded SQL.
...
- **Test-TcpkTempFileToctou** - C37. Predictable temp file TOCTOU (time-of-check to time-of-use) race condition.
...
- **Test-TcpkAppInitVendorOwned** - C25. AppInit_DLLs and AppCertDlls global injection registrations (read-only).
...
- **Test-TcpkComHijack** - C20. Per-user COM CLSID hijack opportunities (T1546.015), plus the two server-side cases: the registered server image exists and is user-writable, and the image is ABSENT while the path it names is one a standard user can create (a dangling registration, which needs no registry write at all). That last case is reported only when the path is under the audited tree, since the registration can name anywhere on the machine; the rest are counted in a census record rather than reported against the vendor. Managed servers registered through mscoree.dll are a known blind spot.
...
- **Test-TcpkHostNameResolution** - C21. Host-side name-resolution posture and shipped-config bare-hostname surface. Rules: `host.llmnr-enabled` MEDIUM (registry DNSClient EnableMulticast != 0), `host.nbtns-enabled` MEDIUM (any NetBT interface with NetbiosOptions != 2), `host.mdns-enabled` LOW (Dnscache EnableMDNS != 0), and `cfg.bare-hostname-endpoint` MEDIUM per shipped config that names a backend by bare hostname (no dot, not a loopback / test alias) via URI scheme (`http://db01`, `mqtt://broker`) or key form (`"Host": "app01"`) - the Windows resolver falls through DNS to LLMNR / NBT-NS on those, letting Responder capture the auth handshake.
...
- **Test-TcpkInstallerHostsWrite** - C23. Installer scripts and MSI custom actions that write to the hosts file.
...
- **Test-TcpkMsiCustomActions** - C28. MSI custom actions that run with elevated (SYSTEM) privileges.
- **Test-TcpkMsiLaunchConditions** - C26. Whether an installer checks the platform it is about to install onto.
- **Test-TcpkPersistenceLoadPoints** - C24. Machine-wide Windows persistence load points not covered by the dedicated autostart checks.
...
- **Test-TcpkPwshProfileAcl** - C20. PowerShell profile.ps1 auto-load points across the six canonical paths (5.1 x64/x86, 7.x, AllUsers + CurrentUser). `loadpoint.pwsh-profile.file-writable` HIGH when the shipped profile file's DACL grants a non-admin principal a right that permits code injection; `loadpoint.pwsh-profile.dir-writable` HIGH when the file does not exist yet and the containing directory is writable (attacker creates it); `loadpoint.pwsh-profile.hijack-source` MEDIUM when the profile dot-sources / Import-Modules a path that lives in a user-writable folder or does not exist. ATT&CK T1546.013.
...
- **Test-TcpkUninstallStringHijack** - C22. Unquoted UninstallString / QuietUninstallString / ModifyPath entries.
- **Test-TcpkUnquotedServicePath** - C03. Classic unquoted-service-path LPE primitive.
- **Test-TcpkVendorDriverAcl** - C19. Vendor kernel-driver attack surface. Confirms every .sys is a real kernel PE (Subsystem=Native + at least one import from ntoskrnl/hal/wdfldr/storport/netio) and then reports non-admin write on the .sys itself, on its parent folder, and on the HKLM\SYSTEM\CurrentControlSet\Services\<Name> key (all HIGH). Also parses shipped INF files (`ServiceType`+`StartType` for kernel-driver installs, missing `CatalogFile=`), greps shipped scripts for `bcdedit /set testsigning on`, maps the .sys basename to a curated vendor dictionary (Silabs / WCH / FTDI / ST / Segger / Nordic / Microchip / Razer / ASUS / Gigabyte / MSI), and explicitly flags Jungo `windrvr6.sys` as a known BYOVD staple.
...
- **Test-TcpkChromiumCleartextStores** - D10. Chromium unencrypted data stores (Local Storage, HTTP cache, IndexedDB).
- **Test-TcpkCredentialLiterals** - D11. Credentials hardcoded as positional string literals in compiled code, which name-based and entropy scanning both miss.
...
- **Test-TcpkFileActivity** - E12. File creates and writes to sensitive paths during an exercise window (ETW).
...
- **Test-TcpkRegistryWrites** - E11. Registry writes by a running process during an exercise window (ETW).
...
- **Test-TcpkUiDataExposure** - E25. Sensitive values surfaced through the UI, and capture hardware the application can be recorded with.
...
- **Test-TcpkNativeCertFlags** - F12. Certificate-validation suppression through the native HTTP stacks and scripting-runtime flags.
...
- **Test-TcpkLogInjection** - C30. Log injection and CRLF log-forging attack surface in logging configuration.
- **Test-TcpkPiiInLogs** - H03. PII patterns in shipped logs / templates / data files.
- **Test-TcpkSecurityEventLogging** - C36. Missing security event logging: absence of audit-trail APIs in first-party code.
...
from this page (329 cmdlets across 19 buckets, 221 of them `Test-*` detection checks).
